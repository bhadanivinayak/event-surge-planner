...
import { Loader2, Zap, CheckCircle2, TrendingUp, Calendar, BarChart3, ArrowRight } from 'lucide-react'

const valuePoints = [
  { icon: Calendar, color: '#f59e0b', text: 'Ramadan shifts 11 days earlier every year — we track it automatically' },
  { icon: TrendingUp, color: '#34d399', text: 'Per-SKU surge forecasts for all GCC events 2025–2027, pre-loaded' },
  { icon: BarChart3, color: '#818cf8', text: 'Confidence bands + order date recommendations based on your lead time' },
  { icon: CheckCircle2, color: '#f472b6', text: 'Connect SAP B1, Zoho, or upload Excel — no complex integrations' },
]
...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    toast.success('Account created! Welcome to Event Surge Planner.')
    router.push('/dashboard')
    router.refresh()
  }
  ...
  const inputStyle = (field: string) => ({
    ...
    borderColor: focused === field ? 'rgba(52,211,153,0.7)' : undefined,
    background: focused === field ? 'rgba(52,211,153,0.07)' : undefined,
    boxShadow: focused === field ? '0 0 0 3px rgba(52,211,153,0.12)' : undefined,
  })
  ...
  const selectStyle = (field: string) => ({
    ...
    borderColor: focused === field ? 'rgba(52,211,153,0.7)' : undefined,
    background: focused === field ? 'rgba(52,211,153,0.07)' : 'rgba(255,255,255,0.04)',
    boxShadow: focused === field ? '0 0 0 3px rgba(52,211,153,0.12)' : undefined,
    border: `1px solid ${focused === field ? 'rgba(52,211,153,0.7)' : 'rgba(255,255,255,0.09)'}`,
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070B14', display: 'flex', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div className="sm-orb" style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle, rgba(52,211,153,0.11) 0%, transparent 70%)', top: -180, left: -120, filter: 'blur(70px)' }} />
        <div className="sm-orb" style={{ position: 'absolute', width: 450, height: 450, borderRadius: '50%', background: 'radial-gradient(circle, rgba(16,185,129,0.09) 0%, transparent 70%)', bottom: -120, right: -80, filter: 'blur(70px)', animationDelay: '4s' }} />
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(52,211,153,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(52,211,153,0.025) 1px, transparent 1px)', backgroundSize: '64px 64px' }} />
      </div>
      ...
      <div className="hidden lg:flex" style={{ flexDirection: 'column', justifyContent: 'space-between', width: '52%', padding: '48px 52px', position: 'relative', zIndex: 10 }}>
        ...
        <div className="sm-fade-up" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 9, background: 'linear-gradient(135deg, #10b981, #34d399)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 18px rgba(52,211,153,0.35)' }}>
            <Zap size={17} color="white" />
          </div>
          <span style={{ color: '#fff', fontSize: 19, fontWeight: 700, letterSpacing: '-0.3px' }}>Event Surge Planner</span>
        </div>

        {/* Hero */}
        <div style={{ maxWidth: 460 }}>
          <div className="sm-fade-up" style={{ animationDelay: '80ms', display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 999, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.22)', marginBottom: 22 }}>
            <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#34d399' }} className="sm-orb" />
            <span style={{ fontSize: 11, color: '#6ee7b7', fontWeight: 500, letterSpacing: '0.04em' }}>Free 30-day trial · No credit card</span>
          </div>
          ...
          <h1 className="sm-fade-up" style={{ animationDelay: '140ms', fontSize: 46, fontWeight: 800, lineHeight: 1.1, letterSpacing: '-1.4px', color: '#fff', marginBottom: 18 }}>
            Forecast GCC events<br />
            <span style={{ background: 'linear-gradient(90deg, #34d399, #10b981, #059669)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>before they happen.</span>
          </h1>

          <p className="sm-fade-up" style={{ animationDelay: '200ms', fontSize: 16, color: 'rgba(148,163,184,0.8)', lineHeight: 1.65, marginBottom: 36 }}>
            Join UAE FMCG distributors who stop running out of stock during Ramadan. Per-SKU surge forecasts for every GCC event, ready in minutes.
          </p>
          ...
          <div className="sm-fade-up sm-glass" style={{ animationDelay: '320ms', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #10b981, #34d399)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16 }}>V</div>
            <div>
              <p style={{ fontSize: 13, color: '#e2e8f0', fontWeight: 500, marginBottom: 2 }}>"We cut our Ramadan stockouts by 60% in the first event cycle."</p>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', position: 'relative', zIndex: 10 }}>
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          <div className="flex lg:hidden sm-fade-up" style={{ alignItems: 'center', gap: 9, justifyContent: 'center', marginBottom: 28 }}>
            ...
            <span style={{ color: '#fff', fontSize: 17, fontWeight: 700 }}>Event Surge Planner</span>
          </div>
          ...
          <div className="sm-fade-up sm-glass-strong" style={{ animationDelay: '100ms', borderRadius: 24, padding: '32px 30px' }}>
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ fontSize: 13, color: 'rgba(148,163,184,0.65)' }}>GCC event-aware demand forecasting — free for 30 days</p>
            </div>
            ...
            <form onSubmit={handleSignup}>
              ...
              <button type="submit" disabled={loading}
                style={{ width: '100%', padding: '13px 20px', borderRadius: 12, border: 'none', background: 'linear-gradient(135deg, #10b981, #059669)', color: '#fff', fontSize: 14, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: loading ? 0.7 : 1, boxShadow: '0 0 20px rgba(16,185,129,0.3)', transition: 'all 0.2s' }}>
                {loading
